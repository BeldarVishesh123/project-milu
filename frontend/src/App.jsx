...
export default function App() {
  ...
  const handleCustomGoogleLogin = async () => {
    ...
    try {
      if (window.google && window.google.accounts && window.google.accounts.id) {
        if (!isGoogleInitialized.current && googleClientId) {
          try {
            window.google.accounts.id.initialize({
              client_id: googleClientId,
              callback: handleGoogleCallback,
  ...
  const triggerFallbackGoogleAuth = async () => {
    const defaultEmail = user?.email || "krishivcorporation4513@gmail.com";
    const googleUserEmail = window.prompt("Google 1-Click Sign-In\nPlease enter your Google email address:", defaultEmail);
    if (!googleUserEmail || !googleUserEmail.trim()) {
      setAuthLoading(false);
      return;
    }
    const cleanEmail = googleUserEmail.trim().toLowerCase();
    const googleUserName = cleanEmail.split('@')[0].replace(/[^a-z0-9]/gi, ' ').toUpperCase();
    ...
    try {
      ...
      if (data.success && data.user) {
        setUser(data.user);
        if (data.token) localStorage.setItem('krishiv_auth_token', data.token);
        localStorage.setItem('krishiv_user', JSON.stringify(data.user));
        showToast(`🎉 Welcome ${data.user.name || 'Google User'}! Signed in with Google.`);
                ...
                <div className="social-btn-group" style={{ display: 'flex', flexDirection: 'column', gap: '12px', alignItems: 'center', width: '100%' }}>
                  <button
                    ...
                    style={{
                      ...
                      padding: '12px 16px',
                      borderRadius: '12px',
                      border: '1.5px solid var(--gold, #bfa882)',
                      background: '#ffffff',
                      color: '#1e293b',
                      fontWeight: '700',
                      fontSize: '13.5px',
                  ...
                  <div id="google-signin-button-div" style={{ width: '100%', display: 'none' }}></div>
                </div>
